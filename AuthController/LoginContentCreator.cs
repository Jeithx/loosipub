...
{
    [HttpPost("LoginContentCreator")]
    public async Task<IActionResult> LoginContentCreator([FromBody] UserForLoginDTO dto)
    {
        try
        {
            var result = await _service.ContentCreatorLogin(dto);
            if (result.Success)
            {
            ...
            return BadRequest(new ErrorDataResult<ReturnContentCreatorDTO>(result.Message));
        }
        catch (Exception ex)
        {
            return BadRequest(new ErrorDataResult<ReturnContentCreatorDTO>(Messages.ProcessFailed));
        }
    }
}